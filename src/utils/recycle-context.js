...
RecycleContext.prototype._forceRerender = function (id, cb) {
  ...
  function newcb() {
    ...
    if (that.autoCalculateSize && that.placeholderClass) {
      ...
      jd.createSelectorQuery().selectAll(newQueryClass.join(',')).boundingClientRect(rect => {
        if (rect.length < count) return
        allrect = rect
        setPlaceholderImage()
      }).exec()
      jd.createSelectorQuery().select('.' + that.itemSizeOpt.queryClass).boundingClientRect(rect => {
        parentRect = rect
        setPlaceholderImage()
      }).exec()
    }
...
RecycleContext.prototype._recalculateSizeByProp = function (list, cb) {
  ...
  if (itemSize.cacheKey) {
    propValueMap = jd.getStorageSync(itemSize.cacheKey) || []
  ...
  function sizeReady(rects) {
    ...
    if (itemSize.cacheKey) {
      jd.setStorageSync(itemSize.cacheKey, propValueMap) // 把数据缓存起来
    }
    if (cb) {
      cb(sizeData)
    }
  ...
  if (calcNewItems.length) {
    ...
    this.page.setData(obj, () => {
      // jd.createSelectorQuery().select(itemSize.componentClass).boundingClientRect(rects => {
      //   compSize = rects;
      //   if (compSize && allItemSize) {
      //     sizeReady();
      //   }
      // }).exec();
      jd.createSelectorQuery().selectAll('.' + itemSize.queryClass).boundingClientRect((rects) => {
        sizeReady(rects)
      }).exec()
    })
  } else {
